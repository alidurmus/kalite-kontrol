...
class Malzemeler extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $search_adi = $this->input->get('search_adi');
        $search_kodu = $this->input->get('search_kodu');

        $where = [];

        if ($search_adi) {
            $where['adi LIKE'] = '%' . $search_adi . '%';
        }

        if ($search_kodu) {
            $where['kodu LIKE'] = '%' . $search_kodu . '%';
        }

        $config['base_url'] = base_url('malzemeler/index');
        $config['total_rows'] = $this->malzemeler_model->get_count($where);
        $config['per_page'] = 20;
        ...
        $items = $this->malzemeler_model->get_limit(
            $where,
            $config['per_page'],
            $page
        );
        ...
        $viewData->search_adi = $search_adi;
        $viewData->search_kodu = $search_kodu;
    ...
    public function update($id)
    {
        ...
        if ($validate) {
            ...
            $update = $this->malzemeler_model->update(['id' => $id], $data);
